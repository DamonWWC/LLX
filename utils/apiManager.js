...
class APIManager {
  ...
  orderManager = {
    ...
    // 批量删除订单
    async deleteOrdersBatch(orderIds) {
      try {
        await api.orderAPI.deleteOrdersBatch(orderIds)
        console.log('[订单管理] 批量删除订单成功，共删除', orderIds.length, '个订单')
        return true
      } catch (error) {
        console.error('[订单管理] 批量删除订单失败:', error.message)
        throw error
      }
    }
  }
